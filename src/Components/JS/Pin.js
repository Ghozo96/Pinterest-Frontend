...
class Pin extends Component {
	...
	savePin = async () => {
		...
		let response = await fetch(
			`${process.env.REACT_APP_HOST_IP}/pin/${this.state.pin_id}/${savecase}`,
			requestOptions
		);
		let data = await response.json();
	...
	follow = async () => {
		...
		let response = await fetch(
			`${process.env.REACT_APP_HOST_IP}/profile/${this.state.owner}/${followcase}`,
			requestOptions
		);
		let data = await response.json();
		if (response.status == 400) {
	...
	addComment = async () => {
		...
		let response = await fetch(
			`${process.env.REACT_APP_HOST_IP}/pin/${this.state.pin_id}/comment`,
			requestOptions
		);
		let data = await response.json();
		console.log(data);
						...
						<div className='metaContainer d-flex flex-column p-3'>
							<div className='optionsBar d-flex align-items-center align-content-center justify-content-between'>
								<div className='d-flex align-items-center align-content-center flex-wrap'>
									...
									<div>
										<button
											type='button'
											className='btn optionsLeft m-1 rounded-pill'>
							...
							<div className='commentsContainer d-flex flex-column  align-content-start mt-3'>
								...
								{this.state.comments.map((mycomments) => {
									return (
										<Comment {...mycomments} key={mycomments.id} />
									);
								})}
